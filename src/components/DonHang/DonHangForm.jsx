...
const SearchInput = ({ value, onChange, options = [], placeholder, label, showAddNew = false, onAddNew }) => {
    ...
    const filteredOptions = options.filter(opt =>
        (opt?.nameDisplay || '').toLowerCase().includes((searchTerm || '').toLowerCase())
    );
    ...
    return (
        <div className="flex items-center gap-3 relative" ref={wrapperRef}>
            ...
            <div className="flex-1 relative">
                {label && <label className="text-[11px] text-gray-500 absolute -top-3 left-0">{label}</label>}
                <input
                    type="text"
                ...
                />
                {searchTerm && (
                    <button onClick={() => { setSearchTerm(''); onChange(''); }} className="absolute right-0 top-0 text-gray-400 hover:text-gray-600 font-bold">&times;</button>
                )}
                {isOpen && (
                    <div className="absolute top-full left-0 w-full mt-1 bg-white border shadow-lg rounded z-[100] max-h-60 overflow-y-auto">
                        {filteredOptions.length > 0 ? (
                            filteredOptions.map(opt => (
                                <div key={opt._id} className="px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer"
                                    onClick={() => { setSearchTerm(opt.nameDisplay); onChange(opt._id); setIsOpen(false); }}
                                >
                                    {opt.nameDisplay}
                                </div>
                            ))
                        ...
                        {showAddNew && (
                            <div onClick={() => { setIsOpen(false); if (onAddNew) onAddNew(); }} className="px-3 py-2 border-t text-green-600 font-medium hover:bg-gray-50 cursor-pointer flex items-center gap-2">
                                <span>+</span> Thêm mới
                            </div>
                        )}
                    </div>
...
const DonHangForm = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        yeuCauHoanThanh: '', henGiao: '',
        danhSachSanPham: [{ loaiDon: 'Mới', sanPham: '', viTri: [], soLuong: 1, mau: '', ghiChu: '' }],
        danhSachPhuKien: [],
        chiDinhBacSi: '', ghiChuChung: '', ghiChuTaiChinh: ''
    });

    const fetchSanPhamData = async () => {
        try {
            const res = await api.get('/sanpham');
            setSanPhamList((res.data.data || res.data || []).map(sp => ({ ...sp, nameDisplay: sp.tenSanPham })));
        ...
        const fetchMasterData = async () => {
            try {
                const [resNk, resBs, resBn] = await Promise.all([
                    api.get('/nhakhoa'), api.get('/nguoilienhe'), api.get('/benhnhan')
                ]);

                setNhaKhoasList((resNk.data.data || resNk.data || []).map(nk => ({ ...nk, nameDisplay: nk.tenGiaoDich || nk.hoVaTen })));
                setAllBacSi((resBs.data.data || resBs.data || []).map(bs => ({ ...bs, nameDisplay: bs.hoVaTen })));
                setAllBenhNhan((resBn.data.data || resBn.data || []).map(bn => ({ ...bn, nameDisplay: bn.hoVaTen })));

                await fetchSanPhamData();
            } catch (err) { console.error("Lỗi tải DB:", err); }
        };
        fetchMasterData();
    }, []);

    const bacSiList = formData.nhaKhoa ? allBacSi.filter(bs => (bs.nhaKhoa?._id || bs.nhaKhoa) === formData.nhaKhoa) : [];
    const benhNhanList = formData.nhaKhoa ? allBenhNhan.filter(bn => (bn.nhaKhoa?._id || bn.nhaKhoa) === formData.nhaKhoa) : [];
    ...
    useEffect(() => {
        if (formData.nhaKhoa) {
            setSelectedNhaKhoaInfo(nhaKhoasList.find(nk => nk._id === formData.nhaKhoa));
        } else {
