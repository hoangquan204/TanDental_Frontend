...
const DonHangPage = () => {
  ...
  const dangSanXuat = donHangs.filter(
    (d) => d.trangThai === "Đang sản xuất"
  ).length;
  const treHen = donHangs.filter(
    (d) =>
      d.henGiao &&
      new Date(d.henGiao) < now &&
  ...
  const filteredDonHangs = donHangs.filter((dh) => {
    ...
    return (
      ma.includes(term) ||
      nhaKhoa.includes(term) ||
      bacSi.includes(term) ||
      benhNhan.includes(term)
    );
  });
      ...
      <div className="flex justify-between items-center mb-4 bg-white p-3 rounded shadow-sm border">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className="bg-green-500 text-white rounded-full hover:bg-green-600 flex items-center justify-center w-8 h-8 font-bold text-lg shadow-sm"
          >
            +
          </button>
          <button
            onClick={() => dispatch(fetchDonHang())}
            title="Tải lại"
